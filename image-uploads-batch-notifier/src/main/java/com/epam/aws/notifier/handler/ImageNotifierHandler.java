package com.epam.aws.notifier.handler;
...
public class ImageNotifierHandler implements RequestHandler<Map<String, Object>, APIGatewayProxyResponseEvent> {

    private static final String SQS_QUEUE_NAME = "image-uploads-notification-queue";
    private static final String SNS_TOPIC_ARN = "arn:aws:sns:us-east-1:435142194056:image-uploads-notification-topic";
    ...
    private final AmazonSNS amazonSNS = AmazonSNSClientBuilder.standard().withRegion(REGION).build();
    private final AmazonSQS amazonSQS = AmazonSQSClientBuilder.standard().withRegion(REGION).build();
    private LambdaLogger log;


    @Override
    public APIGatewayProxyResponseEvent handleRequest(Map<String, Object> inputEvent, Context context) {
        Object source = inputEvent.get(DETAIL_TYPE_PARAM);
        String sourceType = source != null ? String.valueOf(source) : API;
        log = context.getLogger();

        int processedMessages = processQueueMessages();

        log.log(String.format("Function '%s' executed by '%s' has consumed %d messages from '%s' Queue" +
                        " and notified to SNS Topic = %s",
                context.getFunctionName(), sourceType, processedMessages, SQS_QUEUE_NAME, SNS_TOPIC_ARN));
    ...
    private int processQueueMessages() {
        ...
        PublishRequest publishRequest = new PublishRequest()
                ...
                .withSubject("Upload Images Notification")
                .withMessage(message);
